"""Per-project insight from Codex Chronicle session data.

Generates an LLM-synthesized HTML report via `codex exec` (through the
shared codex_cli wrapper). Aggregates session data in Python, sends it
to the model for narrative analysis + HTML generation, writes the result
to ~/.codex-chronicle/projects/<slug>/insight.html and opens it.
...
You will receive structured data about all Codex CLI sessions in this project. Generate a \
...
def generate_insight(project_name: str | None = None):
    ...
    async def _generate():
        res = await spawn_codex(
            prompt=prompt, model=model, fallback_model=fallback, timeout=300,
        )
        if not res.ok:
...
def main():
    parser = argparse.ArgumentParser(description="Per-project Codex Chronicle insight")
