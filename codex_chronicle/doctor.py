...
from .config import (
    chronicle_dir, codex_sessions_dir, config_file,
...
def _count_sessions() -> dict:
    ...
    if sessions_root.exists():
        for jsonl in sessions_root.rglob("rollout-*.jsonl"):
            stem = jsonl.stem
            sid = stem.removeprefix("rollout-") if stem.startswith("rollout-") else stem
            if is_succeeded(sid):
                ok += 1
...
def run(argv: Sequence[str] | None = None) -> int:
    """Entry point. Returns 0 iff diagnostics `ok` is true."""
