"""Generate a unified project story from all Codex Chronicle sessions.

Reads every session markdown file chronologically, sends the full content
to the model for synthesis into a single cohesive narrative. The output is a
...
the full content of every Codex CLI chronicle session in chronological order. Your job is to \
...
def generate_story(project_name: str | None = None):
    ...
    async def _generate():
        res = await spawn_codex(
            prompt=prompt, model=model, fallback_model=fallback, timeout=600,
        )
        if not res.ok:
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate unified project story from Codex Chronicles"
    )
