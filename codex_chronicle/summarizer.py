...
`codex exec` via codex_chronicle.codex_cli.spawn_codex.

Uses --output-schema for structured output validation and --ephemeral
so the summarizer session doesn't appear in the user's history.
"""
...
You are writing a high-fidelity engineering chronicle from a Codex CLI session transcript.
...
- CAPTURE ARCHITECTURE: When architectural patterns are discussed or established, \
document them with the rationale.
- CAPTURE PLANNING: When work is broken into phases or steps, document the plan, \
what order was chosen and why, what was deferred, and how the plan evolved.
- CAPTURE FOLLOW-UPS: When the developer asks clarifying questions, capture both \
the question and what was learned.
- CAPTURE BACK-AND-FORTH: When there's genuine dialogue — pushback, course corrections — \
preserve that exchange.
- WRITE LIKE A DEVELOPER TALKING: The narrative should read like an engineer explaining \
their thought process to a colleague, not a formal report.

Return structured output matching the provided JSON Schema. When in doubt, include more \
technical detail rather than less.

You MUST return valid JSON matching the schema. Do not wrap in markdown fences.
...
def _extract_structured(outer: dict) -> dict | None:
    """Return the structured data from the codex exec response."""
    # Direct case: outer IS the structured dict (--output-schema returned it directly)
    if "is_empty" in outer or "title" in outer:
        return outer

    # Wrapped in a "result" key (some codex versions)
...
async def async_summarize_session(digest: SessionDigest) -> ChronicleEntry:
    """One-shot summarization via codex exec with --output-schema."""
    config = load_config()
    ...
    if result.error_kind is not None:
        ...
        print(f"[codex-chronicle] summarization {result.error_kind.value}: "
    ...
    if data is None:
        if not (outer.get("result") or "").strip() and not outer:
            entry.is_empty = True
            return entry
        entry.is_error = True
        entry.error_kind = ErrorKind.PARSE.value
        entry.error_message = "structured output missing and result not parseable JSON"
        return entry
    ...
    if entry.human_reasoning:
        ...
        for hr in entry.human_reasoning:
            if isinstance(hr, dict):
                lines.append(f"**{hr.get('moment', '')}**")
                if hr.get("reasoning"):
                    lines.append(hr["reasoning"])
                lines.append("")
            else:
                lines.append(f"- {hr}")
    ...
    if entry.follow_ups:
        ...
        for fu in entry.follow_ups:
            if isinstance(fu, dict):
                lines.append(f"**Q: {fu.get('question', '')}**")
                if fu.get("context"):
                    lines.append(f"*Context*: {fu['context']}")
                if fu.get("outcome"):
                    lines.append(f"*Outcome*: {fu['outcome']}")
                lines.append("")
            else:
                lines.append(f"- {fu}")
